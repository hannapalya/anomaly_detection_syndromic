...
For every method, a single alarm threshold is selected on the validation partition by maximising the objective

$$\mathcal{L}(\tau) = 2 \cdot \text{Sensitivity}(\tau) + 3 \cdot \text{Specificity}(\tau), \quad \text{subject to } \text{Specificity}(\tau) \geq 0.97,$$

where $\tau$ is the percentile threshold applied to the anomaly score. This is the only point in the pipeline at which outbreak labels are used; it represents the operational calibration step in which past confirmed outbreaks are used to set alarm sensitivity. The higher weight on specificity reflects the practical requirement that a surveillance system must maintain a low false-alarm rate to remain credible with epidemiologists. Hyperparameters are re-tuned independently for each outbreak magnitude.
...
**BOCPD-residual.** Bayesian Online Changepoint Detection, following the framework of Adams and MacKay (2007), is applied to the Pearson residuals under a Gaussian approximation. The anomaly score combines the negative log-likelihood under the current baseline with the posterior probability that the current run length is zero (i.e., a changepoint has just occurred).
