...
Methods in this family share a common first stage: a Negative Binomial generalised linear model (GLM) is fitted to the training data, with annual harmonic terms and day-of-week indicators as covariates. Training-period outbreak days are excluded from the fit. The fitted model provides a seasonal baseline prediction $\hat{\mu}_t$ and overdispersion parameter $\hat{\alpha}$ for each day $t$. Different post-processing of the Pearson residuals

$$r_t = \frac{y_t - \hat{\mu}_t}{\sqrt{\hat{\mu}_t + \hat{\alpha}\hat{\mu}_t^2}}$$

yields different detectors.
...
**RateChange-residual.** An exponentially weighted moving average (EWMA) is applied to the Pearson residuals. The anomaly score is the z-score of the EWMA, and alarms are raised during positive excursions.
...
Residualisation produces a large, targeted improvement on exactly the signals where the raw-count methods fail. On strongly seasonal signals, Isolation Forest gains +0.146 sensitivity and KNN gains +0.163. On low-seasonal signals the effect is negligible (Isolation Forest) or modestly positive (KNN, +0.041). The single largest improvement is Isolation Forest on bronchitis (signal 13), which rises from 0.358 to 0.860 (+0.502); the only substantial regression is Isolation Forest on cardiac (signal 3), which falls from 0.724 to 0.418 (−0.306), reflecting that residualisation injects estimation noise on a signal whose near-constant baseline carries no seasonal information to remove.

Because the benefit is confined to seasonal signals, a *conditional* policy that applies residual features only when the training-data seasonal amplitude exceeds a threshold (and raw-count features otherwise) captures most of the gain without the downside. Such a policy lifts mean Isolation Forest sensitivity from 0.523 to 0.593 at small magnitude and from 0.678 to 0.743 at medium magnitude, while never underperforming the raw-count baseline on any individual signal. The gain narrows at large magnitude (Isolation Forest +0.018, KNN +0.003 in the always-residual setting), consistent with the broader finding that representation choices matter most when the outbreak signal is weak.
...
The residual-feature ablation (Section 4.6) reinforces this conclusion from a different direction. Changing only the input representation --- replacing raw counts with Pearson residuals from a seasonal baseline, while holding the detector, features, and tuning fixed --- improves sensitivity by +0.146 (Isolation Forest) and +0.163 (KNN) on strongly seasonal signals. The detectors were never the bottleneck on these signals; the raw-count features were, because they confounded the seasonal cycle with outbreaks. This is direct evidence that the representation, not the model, governs performance in this setting.
